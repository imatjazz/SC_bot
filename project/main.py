...
#local imports
import config, api, buttons, tiles
# from .dbmodel import *
...
def store_conversation(sessionInfo):
	filePath = 'C:\\temp\\'
	fileName = 'SC_Bot_Chat_History.csv'
    # sessionInfo = sessionInfo["dialog"]
	df = pd.DataFrame(sessionInfo["dialog"])
	print("*********************** Chat History Info ************************")
	print(df)
	print("*************************************************************")
	df.to_csv(filePath + fileName, sep=',')
	return df
##########################################################################
...
def start(nosession=False):
	...
	return render_template('start.html',current_user = '', messages = messages, tiles = tiles, buttons = buttons, locked = locked)
